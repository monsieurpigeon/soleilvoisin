...
const MAGNIFIER_WIDTH = 340;
const MAGNIFIER_HEIGHT = 132;
const MAGNIFIER_VIEWPORT_PADDING = 8;

function getMagnifierViewportPosition(clientX: number, clientY: number) {
  let left = clientX - MAGNIFIER_WIDTH / 2;
  let top = clientY - MAGNIFIER_HEIGHT / 2;

  const maxLeft = window.innerWidth - MAGNIFIER_WIDTH - MAGNIFIER_VIEWPORT_PADDING;
  const maxTop = window.innerHeight - MAGNIFIER_HEIGHT - MAGNIFIER_VIEWPORT_PADDING;
...
function PageZoomOverlay({
  ...
  const updateMagnifier = useCallback(
    (clientX: number, clientY: number, zoomLevel: number) => {
      ...
      const sourceW = (MAGNIFIER_WIDTH / zoomLevel) * scaleX;
      const sourceH = (MAGNIFIER_HEIGHT / zoomLevel) * scaleY;
      ...
      const { left, top } = getMagnifierViewportPosition(clientX, clientY);
  ...
  const handlePageClick = useCallback(() => {
    ...
    goNext();
  }, [canGoNext, goNext, isTransitioning]);
  ...
  }, [goNext, goPrev, isTransitioning, onClose]);

  useEffect(() => {
    let cancelled = false;
    const cacheKey = `${displayPage}-zoom`;
    ...
    async function render() {
      ...
      try {
        ...
        const maxHeight = Math.min(window.innerHeight * 0.88, 1200);
        const scale = Math.min(
          maxWidth / baseViewport.width,
          maxHeight / baseViewport.height,
        );
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex cursor-pointer items-center justify-center bg-black/88 p-4 backdrop-blur-md"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className="relative flex max-h-full w-full max-w-5xl cursor-default flex-col items-center"
      ...
      >
        <button
          ...
          className="mb-4 cursor-pointer self-end rounded-full border border-zinc-600 bg-zinc-900/90 px-3 py-1.5 text-xs font-medium text-zinc-200 transition hover:border-amber-400/40 hover:text-amber-100"
        >
          Fermer
        </button>

        <div className={`relative w-full ${motionClass[phase]}`}>
          <div
            ref={pageWrapperRef}
            role="button"
            tabIndex={0}
            ...
            className={`mx-auto w-fit max-w-full ${
              canGoNext && !isTransitioning ? "cursor-pointer" : "cursor-default"
            }`}
          ...
          >
            <canvas
              ref={canvasRef}
              className={`mx-auto block h-auto max-h-[min(80svh,1100px)] w-auto max-w-full bg-[#f4efe6] ${rendering ? "opacity-50" : "opacity-100"} transition-opacity`}
            />
          </div>
        </div>

        <div className="mt-6 flex w-full max-w-sm items-center justify-between gap-4">
          <NavButton
            direction="left"
      ...
      {createPortal(
        <div
          className={`pointer-events-none fixed z-[70] overflow-hidden rounded-2xl bg-[#f4efe6] transition-opacity duration-150 ${
...
export function BookViewer() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl">
      <div className="overflow-hidden rounded-xl border border-zinc-800/80 bg-[#1a1410] p-3 sm:p-4">
